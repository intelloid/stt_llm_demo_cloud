/*!
 * wavesurfer.js microphone plugin 4.2.0 (2021-02-18)
  ...
  _createClass(MicrophonePlugin, [{
    ...
    value: function connect() {
      if (this.stream !== undefined) {
        ...
        this.recordSampleRate = this.mediaStreamSource.context.sampleRate; //this.ws = new WebSocket("ws://localhost:3000/intelloid-STT-stream-web/websocket");
      }
    }
    /**
    ...
    value: function reloadBuffer(event) {
      if (!this.paused) {
        ...
        if (this.browser.browser === 'edge') {
          ...
          var downsampledBuffer = this.downsampleBuffer(event.inputBuffer.getChannelData(0), 16000);
          var buffer = new ArrayBuffer(downsampledBuffer.length * 2);
          var ints = new DataView(buffer);
          this.floatTo16BitPCM(ints, 0, downsampledBuffer);
          var chars = new Uint8Array(ints.buffer);
          var b64 = btoa(String.fromCharCode.apply(null, chars)); //this.ws.send(b64);

          this.fireEvent('pcmReady', b64); //this.fireEvent('pcmReady', chars);
        } else {
          ...
          var _chars = new Uint8Array(ints.buffer);

          var b64 = btoa(String.fromCharCode.apply(null, _chars)); //this.ws.send(b64);
    ...
    value: function downsampleBuffer(buffer, exportSampleRate) {
    ...
    key: "mergeBuffers",
    value: function mergeBuffers(bufferArray, recLength) {
